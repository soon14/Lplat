...
public class CaptchaServlet extends HttpServlet implements ICaptchaManager {
	...
	public void init() throws ServletException {
		...
		log.debug("java.io.tmpdir changed: "+System.getProperty("java.io.tmpdir"));
	 ...
	 * @return
	 */
	public static String initCaptchaCode(HttpServletRequest request){
		CaptchaConsumer consumer = new CaptchaConsumer(captchaQueue, waitTime, width, height, charCount);
		Captcha captcha = consumer.getCaptcha();
		HttpSession session = request.getSession();
		if (null != captcha) {
			session.setAttribute(CAPTCHA_SESSION_KEY, captcha.getCaptchCode());
			return captcha.getCaptchCode();
		}
		return null;
	}
