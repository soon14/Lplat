...
public class CacheHttpSession implements HttpSession, Serializable {
	...
	public void invalidate() {
		...
		//增加了清理缓存的动作；
		this.sessionManager.removeSessionFromCache(this.getId());
		
		if(SessionHttpServletRequestWrapper.class.isAssignableFrom(request.getClass())){
		    SessionHttpServletRequestWrapper requestWrapper=(SessionHttpServletRequestWrapper)request;
		    requestWrapper.getRequest().getSession().setAttribute("sessionid", this.getId());
		    requestWrapper.getRequest().getSession().invalidate();
		}
