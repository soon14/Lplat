...
package com.zengshi.paas.resource;
 ...
 */
public class PaasReloadableResourceBundleMessageSource extends AbstractMessageSource
	...
	private final static String DEFAULT_RESOURCE_DIRECTORY="classpath:message";
	private final static String DEFAULT_BASE_PATH="message";
    

    public void setMappingDirectoryLocations(Map<String, String> mappingDirectoryLocations) {
        String[] basenames;
        try {
            basenames = resovleBasenames(mappingDirectoryLocations);
    ...
    public void setDirectoryLocations(String[] directoryLocations) {
        
        
    }
    
    private String[] resovleBasenames(Map<String, String> mappingDirectoryLocations) throws IOException{
        final Set<String> list = new HashSet<String>();
        if(CollectionUtils.isEmpty(mappingDirectoryLocations)){
            mappingDirectoryLocations=new HashMap<String,String>();
            mappingDirectoryLocations.put(DEFAULT_BASE_PATH, DEFAULT_RESOURCE_DIRECTORY);
        }
        Set<Entry<String, String>> entries= mappingDirectoryLocations.entrySet();
        for(Entry<String,String> entry : entries){
            String[] dires=resovleBasename(entry.getKey(),entry.getValue());
            list.addAll(Arrays.asList(dires));
        }
        
        String[] basenames = new String[list.size()];
        return list.toArray(basenames);
    }

    private String[] resovleBasename(String basePath,String directory) throws IOException {
        final Set<String> list = new HashSet<String>();
        PathMatchingResourcePatternResolver resolver = new PathMatchingResourcePatternResolver();
        Resource[] fileResources = resolver.getResources(directory);
        for (Resource res : fileResources) {
            String dir;
            if (UrlResource.class.isAssignableFrom(res.getClass())) {
                UrlResource urlRes = (UrlResource) res;
                dir = urlRes.getURI().toString();
            } else {
                dir = res.getFile().getParent();
            }
            String baseName="";
            if (dir.indexOf(".properties") > -1) {
                
                if(StringUtils.hasText(basePath)){
                    baseName = "classpath*:/"
                            + dir.substring(dir.indexOf(basePath), dir.lastIndexOf("/")) + "/"
                            + getBaseName(res.getFilename());
                }else{
                    baseName = "classpath*:/"+ getBaseName(res.getFilename());
                }
                
            } else {
                if(StringUtils.hasText(basePath)){
                    baseName = "classpath*:/" + dir.substring(dir.indexOf(basePath)) + "/"
                            + getBaseName(res.getFilename());
                }else{
                    baseName = "classpath*:/"+ getBaseName(res.getFilename());
                }
    ...
    private String getBaseName(String name){
        Pattern pattern=Pattern.compile("^([a-zA-Z]+[a-zA-Z0-9_]*)\\.((?:properties)|(?:xml))$");
	...
	protected PropertiesHolder refreshProperties(String filename, PropertiesHolder propHolder) {
		...
		PathMatchingResourcePatternResolver resolver = new PathMatchingResourcePatternResolver();
		Resource[] fileResources = null;//this.resourceLoader.getResource(filename + PROPERTIES_SUFFIX);
        try {
            fileResources = resolver.getResources(filename + PROPERTIES_SUFFIX);
	...
	public void clearCacheIncludingAncestors() {
		clearCache();
		if (getParentMessageSource() instanceof PaasReloadableResourceBundleMessageSource) {
			((PaasReloadableResourceBundleMessageSource) getParentMessageSource()).clearCacheIncludingAncestors();
		}
	}
