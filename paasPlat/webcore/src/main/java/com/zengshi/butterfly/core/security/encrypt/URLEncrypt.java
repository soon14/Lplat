...
import com.zengshi.butterfly.core.util.BinaryHeler;
...
public class URLEncrypt {
	...
	//private final static String CODEC_TABLE = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789*_";
	...
	private static String encode(byte[] keyBytes) {
		...
		for (int i = 0; i < keyBytes.length; i++) {
			...
			BinaryHeler.formatBinary(keyBytes[i], mergrd);
		...
		int l=sbEncoded.length()/10;
		for(int i=1;i<=l;i++) {
			sbEncoded.insert(i*10, "-");
		}
	...
	public static String decode(String code, String characterSet) {
		...
		for (int i = 0; i < code.length(); i++) {
			...
			BinaryHeler.formatBinary(indexBinary, sbBinarys, FIVE_BIT);
	...
	public static void main(String[] args) {
		...
		try {
			...
			while (true) {
				...
				String enCode = URLEncrypt.encode(in);

				String deCode = URLEncrypt.decode(enCode);
				System.out.println();
